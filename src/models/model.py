...
class ModelForSemiStructQA(nn.Module):
    ...
    def forward(
        self, query: Union[str, list], candidates=None, query_id=None, **kwargs: Any
    ):
        """
        Args:
    ...
    def _get_query_emb(
        self, query: str, query_id: int, emb_model: str = "text-embedding-ada-002"
    ):
        if query_id is None:
    ...
    def evaluate(
        ...
        **kwargs: Any,
        ...
        all_pred = (
            torch.ones(max(self.candidate_ids) + 1, dtype=torch.float) * min(pred) - 1
        )
