...
class SemiStructureKB:
        ...
        while len(paths) < size:
            ...
            for i in range(len(node_types)):
                if i == 0:
                    ...
                    node_type_id = list(self.node_type_dict.keys())[
                        list(self.node_type_dict.values()).index(node_types[1])
                    ]
    ...
    def get_tuples(self) -> list:
        ...
        for n_i, e, n_j in tuples_by_id:
            tuples.append(
                (
                    self.node_type_dict[n_i],
                    self.edge_type_dict[e],
                    self.node_type_dict[n_j],
                )
            )
        ...
        for idx in range(self.num_nodes()):
            try:
                net.add_node(
                    idx,
                    label=getattr(
                        self[idx], self.node_type_dict[self.node_types[idx].item()]
                    )[:1],
